...
double func(chrom one){         //Ä¿±êº¯Êý 
    ...
    for(int i=0;i<B;i++){
    	for(int j=0;j<I;j++){
    	    output[i][j] = one.solu[0+3*i]-(log(j+exp(one.solu[2+3*i])))/log(one.solu[1+3*i]);
		}
	}
...
int main(){
    ...
    cur.init();
    //cur.disp();
    for(int i=0;module()/B>=0.02;i++){
    	
	}
	for(int i=0;i<B;i++){
		cout<<cur.solu[0+3*i]<<' '<<cur.solu[1+3*i]<<' '<<cur.solu[2+3*i]<<endl;
	}
}
